...
(function ($, window, document, undefined) {
  ...
  Owl.Workers = [{
    ...
    run () {
      ...
      const view = Math.max(settings.items * 2, 4);
    ...
    run () {
      ...
      const begin = this.coordinates(this.current()) + padding;
      const end = begin + this.width() * rtl;
      let inner; let outer; const matches = []; let i; let n;
  ...
  Owl.prototype.prepare = function (item) {
    const event = this.trigger('prepare', { content: item });
  ...
  Owl.prototype.update = function () {
    ...
    const filter = $.proxy(function (p) { return this[p]; }, this._invalidated);
    const cache = {};
  ...
  Owl.prototype.onDragMove = function (event) {
    ...
    let pull = null;
    const delta = this.difference(this._drag.pointer, this.pointer(event));
    const stage = this.difference(this._drag.stage.start, delta);
  ...
  Owl.prototype.onDragEnd = function (event) {
    const delta = this.difference(this._drag.pointer, this.pointer(event));
    const stage = this._drag.stage.current;
    const direction = delta.x > 0 ^ this.settings.rtl ? 'left' : 'right';
  ...
  Owl.prototype.clones = function (position) {
    ...
    const map = function (index) { return index % 2 === 0 ? even + index / 2 : odd - (index + 1) / 2; };
    ...
    if (object.type === Owl.Type.Event) {
      ...
      if (!$.event.special[object.name].owl) {
        ...
        $.event.special[object.name]._default = function (e) {
          if (_default && _default.apply && (!e.namespace || !e.namespace.includes('owl'))) {
            return _default.apply(this, arguments);
          }
          return e.namespace && e.namespace.includes('owl');
        };
  ...
  $.fn.owlCarousel = function (option) {
    ...
    return this.each(function () {
      const $this = $(this);
      let data = $this.data('owl.carousel');
...
(function ($, window, document, undefined) {
    ...
    this._handlers = {
      'initialized.owl.carousel change.owl.carousel resized.owl.carousel': $.proxy(function (e) {
        ...
        if ((e.property && e.property.name == 'position') || e.type == 'initialized') {
          ...
          let position = (e.property && e.property.value !== undefined ? e.property.value : this._core.current()) + i;
          const clones = this._core.clones().length;
          const load = $.proxy(function (i, v) { this.load(v); }, this);
  ...
  Lazy.prototype.load = function (position) {
    ...
    const $elements = $item && $item.find('.owl-lazy');
    ...
    $elements.each($.proxy(function (index, element) {
      const $element = $(element); let image;
      const url = (window.devicePixelRatio > 1 && $element.attr('data-src-retina')) || $element.attr('data-src') || $element.attr('data-srcset');
...
(function ($, window, document, undefined) {
  ...
  Video.prototype.fetch = function (target, item) {
    ...
    let id = target.attr('data-vimeo-id') || target.attr('data-youtube-id') || target.attr('data-vzaar-id');
    const width = target.attr('data-width') || this._core.settings.videoWidth;
    const height = target.attr('data-height') || this._core.settings.videoHeight;
    ...
    if (url) {
      ...
      if (id[3].includes('youtu')) {
        type = 'youtube';
      } else if (id[3].includes('vimeo')) {
        type = 'vimeo';
      } else if (id[3].includes('vzaar')) {
        type = 'vzaar';
      } else {
  ...
  Video.prototype.thumbnail = function (target, video) {
    ...
    let icon;
    let path;
    const dimensions = video.width && video.height ? 'width:' + video.width + 'px;height:' + video.height + 'px;' : '';
    const customTn = target.find('img');
    let srcType = 'src';
    let lazyClass = '';
    const settings = this._core.settings;
  ...
  Video.prototype.play = function (event) {
    ...
    let item = target.closest('.' + this._core.settings.itemClass);
    const video = this._videos[item.attr('data-video')];
    const width = video.width || '100%';
    const height = video.height || this._core.$stage.height();
    let html;
    let iframe;
...
(function ($, window, document, undefined) {
  ...
  Navigation.prototype.initialize = function () {
    ...
    this._controls.$absolute.on('click', 'button', $.proxy(function (e) {
      const index = $(e.target).parent().is(this._controls.$absolute)
        ? $(e.target).index()
  ...
  Navigation.prototype.update = function () {
    let i; let j; let k;
    const lower = this._core.clones().length / 2;
    const upper = lower + this._core.items().length;
    const maximum = this._core.maximum(true);
    const settings = this._core.settings;
    const size = settings.center || settings.autoWidth || settings.dotsData
      ? 1
  ...
  Navigation.prototype.draw = function () {
    let difference;
    const settings = this._core.settings;
    const disabled = this._core.items().length <= settings.items;
    const index = this._core.relative(this._core.current());
...
(function ($, window, document, undefined) {
  ...
  var Hash = function (carousel) {
    ...
    this._handlers = {
      ...
      'changed.owl.carousel': $.proxy(function (e) {
        if (e.namespace && e.property.name === 'position') {
          ...
          const hash = $.map(this._hashes, function (item, hash) {
    ...
    $(window).on('hashchange.owl.navigation', $.proxy(function (e) {
      ...
      const items = this._core.$stage.children();
      const position = this._hashes[hash] && items.index(this._hashes[hash]);
